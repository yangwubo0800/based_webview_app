...
public class TagAliasOperatorHelper {
                ...
                case DELAY_SEND_ACTION:
                    if(msg.obj !=null && msg.obj instanceof  TagAliasBean){
                        ...
                        if(context!=null) {
                            ...
                            AFLog.e(TAG,"#unexcepted - context was null");
                        }
                    }else{
                        AFLog.w(TAG,"#unexcepted - msg obj was incorrect");
                    }
                    break;
                case DELAY_SET_MOBILE_NUMBER_ACTION:
                    if(msg.obj !=null && msg.obj instanceof  String) {
                        AFLog.i(TAG, "retry set mobile number");
                        sequence++;
    ...
    public void handleAction(Context context,int sequence,String mobileNumber){
        ...
        AFLog.d(TAG,"sequence:"+sequence+",mobileNumber:"+mobileNumber);
    ...
    public void handleAction(Context context,int sequence, TagAliasBean tagAliasBean){
        ...
        if(tagAliasBean == null){
            AFLog.w(TAG,"tagAliasBean was null");
            return;
        }
        ...
        if(tagAliasBean.isAliasAction){
            switch (tagAliasBean.action){
                ...
                case ACTION_SET:
                    ...
                    break;
                default:
                    AFLog.w(TAG,"unsupport alias action type");
                    return;
            }
        }else {
            switch (tagAliasBean.action) {
                ...
                case ACTION_CLEAN:
                    ...
                    break;
                default:
                    AFLog.w(TAG,"unsupport tag action type");
                    return;
            }
        }
    }
    private boolean RetryActionIfNeeded(int errorCode,TagAliasBean tagAliasBean){
        if(!ExampleUtil.isConnected(context)){
            AFLog.w(TAG,"no network");
            return false;
        }
        //返回的错误码为6002 超时,6014 服务器繁忙,都建议延迟重试
        if(errorCode == 6002 || errorCode == 6014){
            AFLog.d(TAG,"need retry");
            if(tagAliasBean!=null){
    ...
    public void onTagOperatorResult(Context context, JPushMessage jPushMessage) {
        ...
        AFLog.i(TAG,"action - onTagOperatorResult, sequence:"+sequence+",tags:"+jPushMessage.getTags());
        AFLog.i(TAG,"tags size:"+jPushMessage.getTags().size());
        init(context);
        ...
        if(jPushMessage.getErrorCode() == 0){
            AFLog.i(TAG,"action - modify tag Success,sequence:"+sequence);
    ...
    public void onCheckTagOperatorResult(Context context, JPushMessage jPushMessage){
        int sequence = jPushMessage.getSequence();
        AFLog.i(TAG,"action - onCheckTagOperatorResult, sequence:"+sequence+",checktag:"+jPushMessage.getCheckTag());
        init(context);
        //根据sequence从之前操作缓存中获取缓存记录
        TagAliasBean tagAliasBean = (TagAliasBean)setActionCache.get(sequence);
        if(tagAliasBean == null){
        ...
        if(jPushMessage.getErrorCode() == 0){
            AFLog.i(TAG,"tagBean:"+tagAliasBean);
            setActionCache.remove(sequence);
    ...
    public void onAliasOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
        AFLog.i(TAG,"action - onAliasOperatorResult, sequence:"+sequence+",alias:"+jPushMessage.getAlias());
        init(context);
        //根据sequence从之前操作缓存中获取缓存记录
        TagAliasBean tagAliasBean = (TagAliasBean)setActionCache.get(sequence);
        if(tagAliasBean == null){
        ...
        if(jPushMessage.getErrorCode() == 0){
            AFLog.i(TAG,"action - modify alias Success,sequence:"+sequence);
            setActionCache.remove(sequence);
    ...
    public void onMobileNumberOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
        AFLog.i(TAG,"action - onMobileNumberOperatorResult, sequence:"+sequence+",mobileNumber:"+jPushMessage.getMobileNumber());
        init(context);
        if(jPushMessage.getErrorCode() == 0){
            AFLog.i(TAG,"action - set mobile number Success,sequence:"+sequence);
            setActionCache.remove(sequence);
        }else{

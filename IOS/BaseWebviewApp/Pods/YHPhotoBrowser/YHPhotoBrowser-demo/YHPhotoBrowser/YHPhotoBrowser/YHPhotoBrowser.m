...
- (void)setupToolbars
{
    ...
    NSInteger iphoneXPadding=isIphoneX?20:0;
    _indexLabel.frame =  CGRectMake(20, self.bounds.size.height - 30-iphoneXPadding, 60, 30);
    _saveButton.frame = CGRectMake(kScreenWidth-70, self.bounds.size.height - 30-iphoneXPadding, 60, 30);
}
...
-(void)configSubView{
    ...
    // do not need to show index and save button, modify by willow 20191028
    //[self setupToolbars];
...
- (void)handlePan:(UIPanGestureRecognizer*) pan
{
    ...
    if (pan.state == UIGestureRecognizerStateBegan)
    {
        ...
        if (x > y) { // 水平移动
            _isHorizontalMoved=YES;
        }else if (x < y){
            _isHorizontalMoved=NO;
        }
        ...
        if(!_isHorizontalMoved){
            ...
            [self getNowImageView].transform= CGAffineTransformConcat(translation, scale);
         }
              ...
              pan.state == UIGestureRecognizerStateFailed)
    {
         ...
         if(fabs(point.y) > 100 || fabs(veloctyPoint.y) > 800){
             
             [UIView animateWithDuration:0.3f animations:^{
                 [self getNowImageView].alpha=0.0f;
                 self.alpha = 0.0f;
             } completion:^(BOOL finished) {
                 [self removeFromSuperview];
             }];
        }else{
             [self getNowImageView].transform = CGAffineTransformIdentity;
             [self getNowImageView].hidden=NO;
         }
